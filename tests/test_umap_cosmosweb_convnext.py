...
def run_probe(embeddings, y, test_size=2000, random_state=42):
    """KNN probe for a single parameter; handles NaN by masking."""
    valid = np.isfinite(y)
    ...
    probe = KNeighborsRegressor(n_neighbors=10)
    probe.fit(X_tr, y_tr)
...
fig.suptitle(
    f"Redshift KNN probe — {N_GALAXIES} HSC galaxies  "
...
scaling_path = f"{OUT_DIR}/scaling_cosmosweb_convnext_{N_GALAXIES}.pdf"
